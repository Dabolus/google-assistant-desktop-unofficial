...
interface FluxStandardNormalActionPayload {
  [key: string]: unknown;
}
type FluxStandardErrorActionPayload = Error;
export type FluxStandardActionPayload =
  | FluxStandardNormalActionPayload
  | FluxStandardErrorActionPayload;

interface FluxStandardCommonAction<T extends string = string, M = unknown>
  extends Action<T> {
  meta?: M;
}
